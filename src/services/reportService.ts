...
import { Lead, TeamMember, Call, FollowUp, SiteVisit } from '../models';
...
export class ReportService {
  ...
  async getConversionFunnel(organizationId: string) {
    ...
    const totalLeads = await Lead.countDocuments({ organizationId: orgIdObj });
    ...
    const siteVisitsCount = await Lead.countDocuments({
      organizationId: orgIdObj,
      status: { $in: ['SiteVisitScheduled', 'Negotiation', 'Booked'] },
    });

    const negotiationCount = await Lead.countDocuments({
      organizationId: orgIdObj,
  ...
  async getLeadsBySource(organizationId: string) {
    ...
    const bookedLeads = await Lead.aggregate([
   ...
   */
  async getResponseTimeAnalytics(organizationId: string) {
    const orgIdObj = new mongoose.Types.ObjectId(organizationId);
    ...
    const agentTimes: Record<string, { name: string; sum: number; count: number }> = {};
    times.forEach((t) => {
      if (t.agentId) {
  ...
  async getAgentPerformance(organizationId: string) {
    ...
    const performanceData = await Promise.all(
      agents.map(async (agent) => {
        ...
        const bookingsCount = await Lead.countDocuments({ 
          organizationId: orgIdObj, 
          assignedAgentId: agent._id, 
          status: 'Booked' 
        });

        const callsMade = await Call.countDocuments({ 
          organizationId: orgIdObj, 
          agentId: agent._id 
        });

        const callsConnected = await Call.countDocuments({ 
          organizationId: orgIdObj, 
          agentId: agent._id, 
          outcome: 'completed' 
        });
   ...
   */
  async exportReportCSV(organizationId: string, reportType: 'leads' | 'agents' | 'funnel') {
    const orgIdObj = new mongoose.Types.ObjectId(organizationId);

    if (reportType === 'leads') {
      const leads = await Lead.find({ organizationId: orgIdObj }).populate({
        path: 'assignedAgent',
        populate: { path: 'profile' },
      });
