...
export class PropertyController {
  ...
  async getProperties(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      if (availability) {
        query.availability = availability as string;
      }
  ...
  async updateProperty(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      const property = await Property.findOne({ _id: id, organizationId: orgId });

      if (!property) {
        return res.status(404).json({ error: 'Property not found or unauthorized' });
      }

      if (updates.title !== undefined) property.title = updates.title;
      if (updates.projectName !== undefined) property.projectName = updates.projectName;
      if (updates.location !== undefined) property.location = updates.location;
      if (updates.address !== undefined) property.address = updates.address;
      if (updates.propertyType !== undefined) property.propertyType = updates.propertyType;
      if (updates.price !== undefined) property.price = parseFloat(updates.price);
      if (updates.size !== undefined) property.size = updates.size;
      if (updates.bedrooms !== undefined) property.bedrooms = parseInt(updates.bedrooms);
      if (updates.bathrooms !== undefined) property.bathrooms = parseInt(updates.bathrooms);
      if (updates.description !== undefined) property.description = updates.description;
      if (updates.amenities !== undefined) property.amenities = Array.isArray(updates.amenities) ? updates.amenities : [];
      if (updates.availability !== undefined) property.availability = updates.availability;
      if (updates.brochureUrl !== undefined) property.brochureUrl = updates.brochureUrl;
      if (updates.images !== undefined) property.images = Array.isArray(updates.images) ? updates.images : [];
