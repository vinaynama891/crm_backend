...
import { Lead, TeamMember, Call, Message, FollowUp, SiteVisit, PropertyShare, Activity } from '../models';
...
export class LeadController {
  ...
  async getLeads(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      // Apply Filters
      if (status) baseQuery.status = status as string;
      if (source) baseQuery.source = source as string;
      if (temperature) baseQuery.temperature = temperature as string;
      if (agentId) baseQuery.assignedAgentId = agentId as string;
  ...
  async getLeadById(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const lead = await Lead.findOne(baseQuery).populate({
        path: 'assignedAgent',
      ...
      const followups = await FollowUp.find({ leadId: lead._id }).sort({ createdAt: -1 });
      const sitevisits = await SiteVisit.find({ leadId: lead._id }).populate('propertyId').sort({ createdAt: -1 });
      const shares = await PropertyShare.find({ leadId: lead._id }).populate('propertyId').sort({ createdAt: -1 });

      // Transform Mongoose document to JSON object and append child collections
      const leadObj = lead.toObject() as any;
      leadObj.activities = activities.map(act => {
        const actObj = act.toObject() as any;
  ...
  async createLead(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const assignedLead = await leadAssignmentService.assignLead({
        organizationId: orgId,
      ...
      if (assignedLead && assignedLead.assignedAgentId) {
        ...
        await callService.initiateCallBridge({
          organizationId: orgId,
  ...
  async updateLead(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const lead = await Lead.findOne(baseQuery);
      if (!lead) {
      ...
      if (propertyType !== undefined) lead.propertyType = propertyType;
      if (budget !== undefined) lead.budget = Number(budget);
      if (preferredLocation !== undefined) lead.preferredLocation = preferredLocation;
   ...
   */
  async callLead(req: AuthenticatedRequest, res: Response) {
    const { id } = req.params;
    const orgId = req.orgId!;
    ...
    try {
      ...
      const result = await callService.initiateCallBridge({
        organizationId: orgId,
   ...
   */
  async shareProperty(req: AuthenticatedRequest, res: Response) {
    const { id } = req.params;
    const orgId = req.orgId!;
  ...
  async assignAgent(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const agent = await TeamMember.findOne({ _id: agentId, organizationId: orgId }).populate('profileId');
      if (!agent) {
        return res.status(404).json({ error: 'Target Sales Agent not found in organization' });
      }

      const profileData: any = agent.profileId;

      lead.assignedAgentId = agent._id as any;
      const updatedLead = await lead.save();
   ...
   */
  async sendWhatsApp(req: AuthenticatedRequest, res: Response) {
    const { id } = req.params;
    ...
    try {
      const result = await messageService.sendWhatsAppTemplate({
        organizationId: orgId,
        leadId: id,
        agentId,
        templateName,
