...
exports.siteVisitController = exports.SiteVisitController = void 0;
...
class SiteVisitController {
    ...
    async getSiteVisits(req, res) {
        try {
            ...
            const visits = await models_1.SiteVisit.find(query)
                .populate('lead')
                .populate('property')
                .populate({
                path: 'agent',
    ...
    async createSiteVisit(req, res) {
        ...
        try {
            ...
            // Validate property
            const property = await models_1.Property.findOne({ _id: propertyId, organizationId: orgId });
            if (!property) {
                return res.status(404).json({ error: 'Property not found in inventory' });
            }
            const visit = await models_1.SiteVisit.create({
                organizationId: orgId,
                leadId,
    ...
    async updateStatus(req, res) {
        ...
        try {
            const visit = await models_1.SiteVisit.findOne({ _id: id, organizationId: orgId })
                .populate('lead')
                .populate('property');
            if (!visit) {
            ...
            if (status === 'Completed') {
                ...
                if (leadData && (leadData.status === 'SiteVisitScheduled' || leadData.status === 'Interested')) {
                    await models_1.Lead.updateOne({ _id: visit.leadId }, { $set: { status: 'Negotiation' } });
                    await models_1.Activity.create({
                        organizationId: orgId,
                        leadId: visit.leadId,
                        agentId: req.teamMember?.id,
...
exports.siteVisitController = new SiteVisitController();
